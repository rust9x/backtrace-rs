...
// dbghelp 5.1 (Version included with Windows XP) doesn't have some of the wide APIs, so we have to
// use the ANSI ones.
#[cfg(target_family = "rust9x")]
#[inline(never)]
...
unsafe fn do_resolve_multibyte(
    ...
    const SIZE: usize = MAX_SYM_NAME as usize + mem::size_of::<SYMBOL_INFO>();
    ...
    let multibyte_ptr = info.Name.as_ptr().cast();
...
#[cfg(target_family = "rust9x")]
unsafe fn convert_multibyte(output: &mut [u16; 260 * 2], input: &[u8]) -> usize {
    let input_len = input.len().min(260) as i32;
    let size =
        MultiByteToWideChar(CP_THREAD_ACP, 0, input.as_ptr(), input_len, output.as_mut_ptr(), 0);

    if size >= 260 * 2 - 1 {
        return 0;
    }

    MultiByteToWideChar(CP_THREAD_ACP, 0, input.as_ptr(), input_len, output.as_mut_ptr(), size)
        as usize
}
